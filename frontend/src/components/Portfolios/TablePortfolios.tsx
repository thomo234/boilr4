...
import {
  ...
  setRefetch,
  deleteItemsByIds,
} from '../../stores/portfolios/portfoliosSlice';
import { useAppDispatch, useAppSelector } from '../../stores/hooks';
import { useRouter } from 'next/router';
import { Field, Form, Formik } from 'formik';
import { DataGrid, GridColDef } from '@mui/x-data-grid';
import { loadColumns } from './configurePortfoliosCols';
...
const TableSamplePortfolios = ({
  ...
  const {
    ...
    loading,
    count,
    notify: portfoliosNotify,
    refetch,
  } = useAppSelector((state) => state.portfolios);
  ...
  useEffect(() => {
    if (portfoliosNotify.showNotification) {
      notify(
        portfoliosNotify.typeNotification,
        portfoliosNotify.textNotification,
      );
    }
  }, [portfoliosNotify.showNotification]);
  ...
  const handleViewAction = (id: string) => {
    router.push(`/portfolios/portfolios-view/?id=${id}`);
  };
  ...
  return (
    <>
      ...
      {portfolios && Array.isArray(portfolios) && !showGrid && (
        <ListPortfolios
...
export default TableSamplePortfolios;
